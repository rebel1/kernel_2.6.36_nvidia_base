...
static const struct tegra_emc_chip shuttle_emc_chips[] = {
	{
		...
		.mem_revision_id2 = -1,
		.mem_pid = -1,
		.table = shuttle_emc_tables,
		.table_size = ARRAY_SIZE(shuttle_emc_tables),
	},
};
#endif
...
{
#if LINUX_VERSION_CODE < KERNEL_VERSION(2,6,38) || LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,39) 
	tegra_init_emc(shuttle_emc_chips, ARRAY_SIZE(shuttle_emc_chips));
#else
