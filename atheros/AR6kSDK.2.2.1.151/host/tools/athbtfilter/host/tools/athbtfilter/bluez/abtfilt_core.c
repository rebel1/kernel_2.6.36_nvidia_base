 ...
 *
 */
static const char athId[] __attribute__ ((unused)) = "$Id: //depot/sw/releases/olca2.2/host/tools/athbtfilter/bluez/abtfilt_core.c#19 $";
    ...
    do {    
	  ...
	  A_UINT32 a2dpOptFlags=0;
            ...
            switch (pInfo->A2DPConnection_LMPVersion) {
                ...
                case 1: /* 1.1 */
                case 2: /* 1.2 */
                    pParamsCmd->info.a2dpParams.a2dpWlanUsageLimit = 10;
			/* For HP basic rate headset or others with audio pause problem, 
				this can be set to 10 to improve audio quality but with 30% peforamnce
				tradeoff, restore back to 30 original value if necessary*/
                ...
                default:
                    ...
                    pParamsCmd->info.a2dpParams.a2dpWlanUsageLimit = 25; 
                    /* There is a lot of noise for the TX side when we running 
                     * a2dp with SE HS205. We need to set it as 25 for 205 
                     * while Jabra 3030 can use 100. The throughput is 14.5 
                     * when using 100. It is about 8.7mbps when set as 25. 
                     * It is 12.5Mbps if it is 50
	    ...
	    A_UINT32 scoOptFlags = 0;
            ...
            if ((pInfo->SCOConnectInfo.LinkType == BT_LINK_TYPE_ESCO) && 
                ...
                if (pInfo->SCOConnectInfo.TxPacketLength <= 30) {
                    ...
                    /* 2-EV3 */
                    scoSlots = 1;
                    scoOptFlags |= (BT_SCO_ALLOW_CLOSE_RANGE_OPT | BT_SCO_FORCE_AWAKE_OPT);
                    A_DEBUG(("     eSCO - 2-EV3\r\n"));
                ...
                scoOptFlags = 0;
                pParamsCmd->info.scoParams.psPollLatencyFraction = 1;
                /* account for RX/TX */
                scoSlots *= 2;
