...
/*redefine version code to bypass wrong version code of bionic lib 
  since cupcake is already using wireless v22 */
#ifdef ANDROID 
#undef LINUX_VERSION_CODE 
#define   LINUX_VERSION_CODE (((2) << 16) + ((6) << 8) + (27))
#endif
...
{
  A_CHAR *  end;        /* End of the list */
  A_CHAR *  current;    /* Current event in list of events */
  A_CHAR *  value;      /* Current value in event */
} event_list;
...
{
#ifdef ANDROID
    fprintf(stderr, "usage:\n%s <output log file> <record limit>\n", progname);
#else
    fprintf(stderr, "usage:\n%s <output log file>\n", progname);
#endif 
    exit(-1);
...
#endif /* !Android */
#ifdef ANDROID
    dbgRecLimitStr=argv[2];

    /* Get the file name for dbglog header file */
	snprintf(dbglogfile, FILENAME_LENGTH_MAX, "%s/%s", "/system/wifi/", DBGLOG_FILE);
    /* Get the file name for dbglog id header file */
	snprintf(dbglogidfile, FILENAME_LENGTH_MAX, "%s/%s", "/system/wifi/", DBGLOGID_FILE);
#else
...
#ifdef ANDROID
	    printf("rcvEvent:app_reportError_event_rx restart driver\n");
		system("rmmod ar6000;insmod /system/wifi/ar6000.ko");
#else
